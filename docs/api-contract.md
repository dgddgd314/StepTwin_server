...
Current walking segments are generated by the in-process demo pedestrian router. Seoul Open Data
polyline/graph data is expected to be injected separately into the pedestrian graph import API, then
used by the pgRouting walking router once that router is wired into route preview. The Android
